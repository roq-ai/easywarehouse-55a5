...
import { getBasketItems } from 'apiSdk/basket-items';
...
function DashboardPage() {
  ...
  const basketStat = useSWR<PaginatedInterface<BasketInterface>>(
    () => `/baskets`,
    () => getBaskets({ limit: 1, offset: 0 }),
  );
  ...
  const basketItemStat = useSWR<PaginatedInterface<BasketItemInterface>>(
    () => `/basket-items`,
    () => getBasketItems({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Baskets',
      result: basketStat?.data?.totalCount,
      isLoading: basketStat.isLoading,
    ...
    {
      title: 'Basket Items',
      result: basketItemStat?.data?.totalCount,
      isLoading: basketItemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
