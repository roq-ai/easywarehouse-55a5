...
import { getBasketItemById, updateBasketItemById } from 'apiSdk/basket-items';
...
function BasketItemEditPage() {
  ...
  const handleSubmit = async (values: BasketItemInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateBasketItemById(id, values);
      mutate(updated);
      resetForm();
