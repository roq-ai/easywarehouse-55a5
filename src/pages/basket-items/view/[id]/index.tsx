...
import { getBasketItemById } from 'apiSdk/basket-items';
import { BasketItemInterface } from 'interfaces/basket-item';

function BasketItemViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<BasketItemInterface>(
    () => (id ? `/basket-items/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('basket_item', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/basket-items/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('basket', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Basket"
                    text={
                      <Link as={NextLink} href={`/baskets/view/${data?.basket?.id}`}>
                        {data?.basket?.name}
                      </Link>
                    }
                  />
